...
export default class Modal extends Component {
    ...
    elementOrParentIsFloatingMenu = target => {
        if (target && typeof target.closest === 'function') {
            return this.props.selectorsFloatingMenus.some(selector => target.closest(selector))
        }
        const applySelector = selector => target[matchesFuncName](selector)
        // Alternative if closest does not exist.
        while (target) {
            if (typeof target[matchesFuncName] === 'function') {
                if (this.props.selectorsFloatingMenus.some(applySelector)) {
                    return true
                }
            }
    ...
    focusButton = evt => {
        const primaryFocusElement = evt.currentTarget.querySelector(this.props.selectorPrimaryFocus)
    ...
    handleTransitionEnd = evt => {
        if (this.outerModal.offsetWidth && this.outerModal.offsetHeight && this.beingOpen) {
    ...
    render() {
        ...
        const onSecondaryButtonClick = onSecondarySubmit || onRequestClose
        ...
        const modalButton = (
            <button className="bx--modal-close" type="button" onClick={onRequestClose} ref={this.button}>
                <Icon icon={iconClose} className="bx--modal-close__icon" description={iconDescription} />
            </button>
        )
                ...
                {!passiveModal && (
                    <div className="bx--modal-footer">
                        <div className="bx--modal__buttons-container">
                            <Button kind={danger ? 'tertiary' : 'secondary'} onClick={onSecondaryButtonClick}>
                                {secondaryButtonText}
                            </Button>
                            <Button
